...
\begin{frame}{Berkovich geometry}
	...
	\only<1,2>{\begin{align*}
		...
		C \longmapsto& \text{$C\an =$ closed points of $C$? }\\
			     &\text{\phantom{$C\an = $ closed points of $C \cup $ norms on $K(C)$}}
  .\end{align*}}
	\only<3>{\begin{align*}
		(\cdot )\an: K\text{-curve} \longrightarrow& K\text{-analytic space} \\
		C \longmapsto& \text{\redout{$C\an =$ closed points of $C$}}\\
		  &\text{$C\an =$ closed points of }C \cup \text{ norms on } K(C)
  .\end{align*}}
\end{frame}
