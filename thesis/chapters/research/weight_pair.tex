\todo{proof that $l_0: r \mapsto B(0, |\pi|^{-r})$ is an isometry}
...
The special fiber on one the first affine chart is \[
...
The proof of the following two results depend heavily on \cite[thm.\ 3.2.3]{bakerWeightFunctionsBerkovich2016}. 
	The metric on $\mathbb{H}(\pro^{1}_K)$ we defined in \cref{def:metric_dual_graph} (the stable metric) differs from the metric in used in \cite{bakerWeightFunctionsBerkovich2016} (the potential metric). 
	...
	Let $K_C^{-}$ be the divisor that contains all points of $\omega$ and $\mathscr C$ a snc-model of the pair $(\mathscr C, K_C^{-})$.  
...
\begin{proposition}\label{prop:model_P_is_KS}
	Let $\omega$ be as in \cref{lem:unique_form_pair} and $\mathscr P$ the model constructed in \cref{sec:finding_a_minimal_snc_model}.
