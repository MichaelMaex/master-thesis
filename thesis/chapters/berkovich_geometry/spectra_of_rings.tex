...
The ordinary spectrum of a ring starts with the idea that $\spec L$ is a point, for a field $L$, as in the opposite category or rings, fields behave like points. 
Then the points in $\spec A$ are all the images of points $\spec L \to \spec A$ for some morphism $A \to L$. 
The Berkovich spectrum uses the same idea, but in this theory the starting point is that normed fields are the points. 
Then all the points on $\mathcal{M} (A)$ are given by ``inclusions of points'' $A \to L$ with the equivalence relation above. 
